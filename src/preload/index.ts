...
try {
  ...
  contextBridge.exposeInMainWorld('electronAPI', {
    ...
    transcription: {
      ...
      onUpdate: (callback: (data: { transcript: string, isFormatted: boolean }) => void) => {
        const wrappedCallback = (_event: Electron.IpcRendererEvent, data: { transcript: string, isFormatted: boolean }) => callback(data);
